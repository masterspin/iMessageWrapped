...
def create_graphs(messages, progressText, loading, label):
    ...
    if(len(average_response) > 0):
        ...
        # averageResponseFrame = tk.Frame(app)
        # averageResponseFrame.pack(side=tk.TOP, expand=True)
        # sns.set(style="dark")

        # # Create the plot with 'hue' parameter instead of 'palette'
        # averageResponseax = sns.barplot(x=averageResponseValues, y=averageResponseKeys, orient='h', palette='Set1', hue=averageResponseKeys, dodge=False, legend=False)
        # averageResponseax.set_title('Average Response Time (Minutes)', color='#218aff')  # Set title color
        # averageResponseax.patch.set_facecolor("#242424")

        # # Customize plot aesthetics
        # averageResponsefig = plt.gcf()
        # averageResponsefig.patch.set_facecolor("#242424")  # Set figure background to app's background color
        # averageResponseax.xaxis.label.set_color('#59DFFF')  # Set x-axis label color
        # averageResponseax.yaxis.label.set_color('#59DFFF')   # Set y-axis label color     # Set x-axis label

        # # Rotate y-axis labels to make them visible
        # averageResponseax.set_yticklabels(averageResponseKeys, ha='right', color='#218aff')

        # custom_font = FontProperties(family='Arial', weight='bold', style='italic', size=10)

        # # Display values on top of bars
        # for p in averageResponseax.patches:
        #     averageResponseax.annotate(f'{p.get_width():.0f}', (p.get_x() + p.get_width(), p.get_y() + p.get_height() / 2),
        #                 ha='left', va='center', color='#39ff5a', xytext=(5, 0), textcoords='offset points', fontproperties=custom_font)
        
        # # Set font for x-axis label
        # averageResponseax.xaxis.label.set_fontproperties(custom_font)

        # # Set font for y-axis label
        # averageResponseax.yaxis.label.set_fontproperties(custom_font)
        # averageResponseax.title.set_fontproperties(custom_font)

        # # Set font for y-axis tick labels
        # for label in averageResponseax.get_yticklabels():
        #     label.set_fontproperties(custom_font)
        
        # for label in averageResponseax.get_xticklabels():
        #     label.set_color('#39ff5a')
        #     label.set_fontproperties(custom_font)

        # averageresponse_graph_frame = tk.Frame(average_response_tab)
        # averageresponse_graph_frame.pack(side=tk.TOP, expand=True)

        # averageresponse_canvas = FigureCanvasTkAgg(averageResponsefig, master=averageresponse_graph_frame)
        # averageresponse_canvas.draw()
        # averageresponse_canvas.get_tk_widget().pack(expand=True)

        # averageresponse_toolbar = NavigationToolbar2Tk(averageresponse_canvas, averageresponse_graph_frame)
        # averageresponse_toolbar.configure(background='#242424')  # Set background and foreground colors
        # averageresponse_toolbar.update()
        # averageresponse_toolbar.pack()

        x_values_1 = [1, 2, 3, 4, 5]
        y_values_1 = [10, 15, 13, 18, 20]

        # Create a figure and axes for Graph 1
        fig1, ax1 = plt.subplots()
        ax1.plot(x_values_1, y_values_1, 'b-', label='Graph 1')
        ax1.set_title('Graph 1')
        ax1.set_xlabel('X-axis')
        ax1.set_ylabel('Y-axis')
        ax1.legend()

        canvas1 = FigureCanvasTkAgg(fig1, master=tabview.tab("Average Response Time"))
        canvas1.draw()
    ...
    if(len(sorted_game_pigeon_freq) > 0):
        game_pigeon_tab = tabview.add("Game Pigeon")
        ...
        # Rotate y-axis labels to make them visible
        ax.set_yticklabels(gamePigeonKeys, ha='right', color='#218aff')
        ...
        graph_frame = tk.Frame(game_pigeon_tab)
...
def read_messages(db_location, addressBookData, progressText, loading, self_number='Me', human_readable_date=True):
    ...
    query += f" WHERE message.date >= 694224000000000000 ORDER BY message.date DESC LIMIT 2000"
